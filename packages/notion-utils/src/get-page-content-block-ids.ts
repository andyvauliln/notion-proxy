...
export const getPageContentBlockIds = (
  ...
  function addContentBlocks(blockId: string) {
    ...
    if (
      blockId !== rootBlockId &&
      (type === 'collection_view_page' || type === 'page')
    ) {
      // ignore the content of other pages and collections
      return
    }
    // if (blockId !== rootBlockId && type === 'page' && block.parent_table === 'collection' && block.properties.title) {
    //   contentBlockIds.add(content[0])
    //   console.log(content[0], '________-','data' );
