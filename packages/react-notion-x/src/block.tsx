    ...
    case 'page':
      if (level === 0) {
        ...
        if (fullPage) {
          ...
          const datePropertyId = Object.keys(collection.schema).find(
            (key) => collection.schema[key]?.name?.toLowerCase() === "date"
          )
          const tagsPropertyId = Object.keys(collection.schema).find(
            (key) => collection.schema[key]?.name?.toLowerCase() === "tags"
          )
          console.log(isBlogPost, datePropertyId, tagsPropertyId, block.properties?.[datePropertyId], block,  "-------------Ou");
